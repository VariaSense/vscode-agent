...
export class ViewProvider implements vscode.WebviewViewProvider {
    ...
    public static readonly viewType = 'variasense.chatView';
    ...
    public resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                case 'askLLM':
                    {
                        if (!data.value && (!data.attachments || data.attachments.length === 0)) { return; }
                ...
                case 'updateSettings':
                    {
                        if (data.provider) {
                            await ConfigManager.setProviderType(data.provider);
                        }
                        if (data.baseUrl) {
                            await ConfigManager.setLocalBaseUrl(data.baseUrl);
                        }
                        if (data.apiKey) {
                            ...
                            const currentProvider = data.provider || ConfigManager.getProviderType();
                            if (currentProvider === ProviderType.OpenAI) {
                                await ConfigManager.setOpenAIKey(data.apiKey);
                            }
                        }
    ...
    private async sendSettingsToWebview() {
        ...
        if (models.length > 0) {
            if (type === ProviderType.Local && !localConfig.model) {
                localConfig.model = models[0];
            } else if (type === ProviderType.OpenAI && !openaiConfig.model) {
                openaiConfig.model = models[0];
            }
        }
        const currentModel = (type === ProviderType.Local ? localConfig.model : openaiConfig.model) || "Unknown Model";
        this._view.webview.postMessage({ type: 'updateModelStatus', model: currentModel });
    }

    private async handleUserMessage(userMessage: string, agentMode: boolean, attachments?: any[]) {
        if (!this._view) { return; }
        ...
        try {
            // Build Prompt
            let prompt: string | any[] = userMessage;
...
Always use forward slashes for paths. relative paths are relative to workspace root.`;
    ...
    */
    ...
    private async processAgentResponse(response: string) {
